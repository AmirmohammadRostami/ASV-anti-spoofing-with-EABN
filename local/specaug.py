...
#     print('Squeezed query_points', unstacked_query_points[0].shape, unstacked_query_points[1].shape)
    ...
    for dim in index_order:
        ...
        max_floor = torch.tensor(
            size_in_indexing_dimension - 2, dtype=query_type).cuda()
    ...
    def gather(y_coords, x_coords, name):
        ...
        gathered_values = torch.gather(
            flattened_grid.t().cuda(), 1, linear_coordinates).cuda()
...
def freq_mask(spec, F=80, num_masks=1, replace_with_zero=True):
    cloned = spec.clone()
    num_mel_channels = cloned.shape[2]
    con = False
    for i in range(0, num_masks):
        f = random.randrange(F//4, F)
        ...
        if (replace_with_zero):
            ...
            cloned[0][:, f_zero:mask_end] = torch.randn(
                (mask_end-f_zero))*torch.normal(cloned[0].mean(1), cloned[0].std(1)).unsqueeze(1).to('cpu')
...
def time_mask(spec, T=20, num_masks=1, replace_with_zero=True):
    ...
    for i in range(0, num_masks):
        ...
        if (replace_with_zero):
            ...
            cloned[0][t_zero:mask_end, :] = torch.randn(
                mask_end-t_zero, 1)*torch.normal(cloned[0].mean(0), cloned[0].std(0)).to('cpu')
    return cloned, con


def augmentation(batch_x, b_y, specaug_prob = 0.25):
    aug_bonaf, aug_t = 0, 0
    for i in range(len(batch_x)):
        ...
        con = False
        prob = specaug_prob
        if abs(np.random.random()) <= prob:
            temp, con = freq_mask(temp)
        prob = specaug_prob
        if abs(np.random.random()) <= prob:
            temp, con = time_mask(temp)
            # con = True
        batch_x[i] = temp
        # if con:
        #     aug_t += 1
        #     if b_y[i] == 1:
        #         aug_bonaf += 1
        #         b_y[i] = 0  # change to Attack in Binary mode
        #     # # change to Unk Attack in Multi class
    return batch_x, b_y, aug_bonaf, aug_t
